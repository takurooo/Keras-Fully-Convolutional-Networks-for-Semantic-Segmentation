...
import os
import sys
from keras.layers import Input, Conv2D, Conv2DTranspose, Add, Activation, MaxPooling2D, Dropout, UpSampling2D, concatenate
...
def build(classes=21, input_shape=(224, 224, 3), weights_path=None, weight_decay=0., drop_rate=None, bilinear=False):
    ...
    x = concatenate([p3, u4, u7])
    x = Conv2D(classes, 1, activation='relu', name='conv_final_2',
               kernel_regularizer=l2(weight_decay),
    ...
    if bilinear:
        ...
        x = Conv2DTranspose(classes, 16, activation='relu',
                            strides=8, padding='same', name='upscore_final_x8',
                            kernel_regularizer=l2(weight_decay),
